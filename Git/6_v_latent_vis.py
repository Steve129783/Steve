...
DEFAULT_NAMES = ["correct", "high", "low"]
...
scatter_collections = []  # 用来后面查 artist 对应哪一组
for gid in unique_ids:
    mask = (all_gid == gid)
    idxs = np.where(mask)[0]                   # 全局索引数组
    pc = ax.scatter(mu_pca[mask, 0], mu_pca[mask, 1],
                    label=group_names[gid], s=20, picker=True)
    pc._dataset_indices = idxs                  # 挂上这个属性
    scatter_collections.append(pc)
...
# —— 点击后显示对应原图 —— #
def on_pick(event):
    pc = event.artist                           # 被点击的 PathCollection
    local_inds = event.ind                      # 它内部的点的局部索引列表
    global_inds = pc._dataset_indices           # 我们先前挂上的全局索引
    ds_idx = int(global_inds[local_inds[0]])    # 取出真正的 ds 索引
