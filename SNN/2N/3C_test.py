...
# Which channel(s) of the first conv layer to keep
desired_channels = [15]


# =========================
# 2. Load group id → name
# =========================
...
# =========================
# 5. Freeze & mask first conv layer
# =========================
def apply_mask_to_first_layer(model, desired_channels):
    conv1, bn1 = model.model[0], model.model[1]
    out_ch = conv1.out_channels
    # build mask: only keep desired_channels
...
# =========================
# 6. Extract features & compute contrast
# =========================
def extract_features_and_contrast():
    ...
    # freeze & mask the first conv layer now
    apply_mask_to_first_layer(model, desired_channels)
...
# =========================
# 7. Find best dim & PC correlations
# =========================
...
if __name__ == "__main__":
    ...
    for rank, dim in enumerate(sorted_idx, start=1):
        ranks[dim] = rank
    df_var = pd.DataFrame({'rank': ranks, 'dim': np.arange(D), 'variance': variances})
    var_csv = rf'E:\Project_SNV\2N\c2_cache\{file_name}\variance_rank.csv'
    df_var.to_csv(var_csv, index=False)
    print(f"Saved variance ranking to {var_csv}")

    # export hidden‑dim correlation ranking
    ...
    for rank, dim in enumerate(sorted_idx, start=1):
        ranks_corr[dim] = rank
    df_corr = pd.DataFrame({'rank': ranks_corr, 'dim': np.arange(D), 'pearson_r': all_corrs})
    corr_csv = rf'E:\Project_SNV\2N\c2_cache\{file_name}\hidden_dim_correlation_rank.csv'
    ...
    # plot H[:,best_dim] vs contrast with per‐group limit
    idxs = []
    ...
    sc = ax.scatter(H[idxs, best_dim], contrast[idxs],
    ...
    def on_pick(event):
        ind0 = event.ind[0]
        idx  = idxs[ind0]
        hl, = ax.plot(H[idx, best_dim], contrast[idx],
